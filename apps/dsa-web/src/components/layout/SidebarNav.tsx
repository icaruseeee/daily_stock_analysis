...
export const SidebarNav: React.FC<SidebarNavProps> = ({ collapsed = false, onNavigate, variant = 'default' }) => {
  ...
  const itemBaseClass = cn(
    'group relative flex h-9 w-full items-center overflow-hidden rounded-md border border-transparent text-body-sm leading-none text-ink-muted transition-all no-underline',
    isRail
      ? 'gap-2.5 px-3'
      : collapsed
        ? 'justify-center px-0'
    ...
    <div className="flex h-full w-full flex-col">
      <div
        className={cn(
          ...
          isRail ? 'mb-5 gap-2 px-1 pt-1' : 'mb-4 gap-2 px-1',
          collapsed ? 'justify-center' : ''
        )}
      >
        <div
          className={cn(
            'flex items-center justify-center bg-accent text-white w-full',
        ...
        {!collapsed ? (
          <p className={cn('min-w-0 truncate', isRail ? 'text-[0.95rem] leading-none' : 'text-sm')}>DSA</p>
        ) : null}
      </div>

      <nav className={cn('flex flex-col gap-1.5 w-full', isRail ? '' : 'flex-1')} aria-label={t('layout.mainNav')}>
        {navItems.map(({ key, labelKey, to, icon: Icon, exact, badge }) => {
          ...
          >
            {({ isActive }) => (
              <>
                <Icon className={cn(itemIconClass, isActive ? 'text-accent' : 'text-current')} />
                ...
                {badge === 'completion' && completionBadge ? (
                  <StatusDot
                    ...
                    className={cn(
                      'absolute right-3 border-2 border-surface-2',
